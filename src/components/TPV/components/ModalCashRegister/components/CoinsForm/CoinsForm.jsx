...
const CoinsForm = ({ money, reference, onChangeText, reset }) => {
  ...
  return (
    <>
      <InputGroup title="MONEDAS" restore="Restaurar" onClick={() => reset()} />

      {money.map((m) => {
        if (m.type === 'coins') {
          return (
            <div className="tw-flex tw-justify-between tw-px-2 tw-mt-1">
              {m.component}
